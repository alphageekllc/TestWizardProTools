...
def process(test_file_url, test_env='test001'):
    """
    ...
    response = (sfrest.upload_github_test_file(test_file_url, access_token, instance_url)).json()
    print(response['id'])
    ...
    if(response['success'] == True):
        print(ci.colored(f"Successfully uploaded and processed test file '{test_file_url}'.", "green"))
    else:
        print(ci.colored(f"Failed to upload and process test file '{test_file_url}'.", "red"))
