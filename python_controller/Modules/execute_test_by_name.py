...
def process(test_name, test_env='test001', timeout=60):
    """
    Executes test by name, waits for response within timeout and displays results
    ...
    json_response = http_execute_test_by_name(test_name, access_token, instance_url)
    ...
    while True:
        json_response   = http_get_test_status(test_id, access_token, instance_url)
        elapsed_time    = time.time() - start_time
        print(ci.colored(f"Executing Test by Name Status: {json_response['status']} - {elapsed_time} s...", "blue"))
    ...
    if json_response['status'] == 'TEST PASSED':
        print(ci.colored(f"Test: '{test_name}' - {json_response['status']}", "green"))
    else:
        ci.print_json(data=json_response)     
        print(ci.colored(f"Test: '{test_name}' - {json_response['status']}", "red"))
...
def http_execute_test_by_name(test_name, access_token, instance_url):
    headers     = {
...
def http_get_test_status(test_id, access_token, instance_url):
    headers     = {
        'Authorization': 'Bearer ' + access_token,
        'Content-Type': 'application/json'
    }
